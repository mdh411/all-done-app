...
def create_task():
    if not request.json or not validate_task(request.json):
        abort(400,
              ("Invalid task data. 'name' field is required and must be a "
               "non-empty string."))
...
def edit_tasks(task_id):
    ...
    if not request.json or not validate_task_update(request.json):
        abort(400, (
            "Invalid task data. 'name' field must be a non-empty string "
            "if provided and 'checked' field must be a boolean "
            "if provided."
        ))
